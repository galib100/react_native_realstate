...
export const FeaturedCard = ({item:{name,address,price,image,rating}, onPress }: props) => {
...
export const Card = ({item:{name,address,price,image,rating},onPress }: props) => {
    return (
        <TouchableOpacity onPress={onPress} className="flex-1 w-full mt-4 py-4 rounded-lg bg-white shadow-lg shadow-black-100/70 relative">
            <View className='flex flex-row items-center absolute px-2 top-5 right-5 bg-white/90 p-1 rounded-full z-50 '>
                <Image source={icons.star} className='size-3.5' />
                <Text className='text-xs font-rubik-bold text-primary-300 ml-0.5 '>{rating}
                </Text>
            </View>
